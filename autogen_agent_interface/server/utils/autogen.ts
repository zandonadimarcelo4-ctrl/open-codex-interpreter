    ...
    if (intent.type === "action" || intent.type === "command") {
      try {
        ...
        const pythonScript = [
          ...
          "",
          `# Verificar se o módulo interpreter existe`,
          `try:`,
          `    import interpreter`,
          `    print(f"[DEBUG] Módulo interpreter importado: {interpreter}", file=sys.stderr)`,
          ...
          `    print(f"[DEBUG] interpreter_path: {interpreter_path}", file=sys.stderr)`,
          `    raise`,
          "",
          `# Importar Interpreter usando o módulo já importado`,
          `# Isso garante que os imports relativos dentro de interpreter funcionem`,
          `try:`,
          ...
          `        print(f"[DEBUG] Interpreter importado diretamente", file=sys.stderr)`,
          `    except ImportError:`,
          `        raise e`,
          "",
