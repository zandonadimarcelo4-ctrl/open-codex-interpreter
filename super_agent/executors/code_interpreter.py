...
class CodeInterpreter:
  ...
  def __init__(self, language, debug_mode):
    ...
    self.code = ""  # Adicionar atributo code
    self.output = ""  # Adicionar atributo output
    self.active_block = None  # Adicionar atributo active_block
    self.done = None  # Adicionar atributo done
  ...
  def update_active_block(self):
      ...
      # Display it (only if active_block is set, otherwise just update output)
      if self.active_block:
          self.active_block.active_line = self.active_line
          self.active_block.output = self.output
          self.active_block.refresh()
      # Se não há active_block, apenas atualizar self.output (já feito acima)
  ...
  def run(self):
    ...
    # Get code to execute (from self.code if active_block not set)
    if self.active_block:
        self.code = self.active_block.code
    # Se não há active_block, usar self.code diretamente
